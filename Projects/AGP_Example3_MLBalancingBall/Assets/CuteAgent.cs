...
public class CuteAgent : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        if (useVecObs)
        {
            ...
            sensor.AddObservation(ball.transform.localScale);
    ...
    public void SetBall()
    {
        ...
        var scale = Random.Range(0.2f, 3);
        m_BallRb.mass = scale;
        scale = Random.Range(0.2f, 3);
        if(Random.value < 0.7f)
        {
            scale = Random.Range(2.1f, 3);
        }
        ball.transform.localScale = new Vector3(scale, scale, scale);
    }
